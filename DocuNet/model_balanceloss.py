...
class DotProductMatrixAttention(nn.Module):
    def forward(self, matrix_1: torch.Tensor, matrix_2: torch.Tensor) -> torch.Tensor:
        return torch.matmul(matrix_1, matrix_2.transpose(1, 2))


class CosineMatrixAttention(nn.Module):
    def forward(self, matrix_1: torch.Tensor, matrix_2: torch.Tensor) -> torch.Tensor:
        matrix_1_norm = F.normalize(matrix_1, p=2, dim=-1)
        matrix_2_norm = F.normalize(matrix_2, p=2, dim=-1)
        return torch.matmul(matrix_1_norm, matrix_2_norm.transpose(1, 2))


class BilinearMatrixAttention(nn.Module):
    def __init__(self, matrix_1_dim: int, matrix_2_dim: int, use_input_biases: bool = False):
        super().__init__()
        ...
        self.weight = nn.Parameter(torch.empty(matrix_1_dim, matrix_2_dim))
        nn.init.xavier_uniform_(self.weight)

    def forward(self, matrix_1: torch.Tensor, matrix_2: torch.Tensor) -> torch.Tensor:
        if self.use_input_biases:
            ones_1 = matrix_1.new_ones(matrix_1.size(0), matrix_1.size(1), 1)
            ones_2 = matrix_2.new_ones(matrix_2.size(0), matrix_2.size(1), 1)
            matrix_1 = torch.cat([matrix_1, ones_1], dim=-1)
            matrix_2 = torch.cat([matrix_2, ones_2], dim=-1)

        intermediate = torch.matmul(matrix_1, self.weight)
        scores = torch.matmul(intermediate, matrix_2.transpose(1, 2))
        return scores
