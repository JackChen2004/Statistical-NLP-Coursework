...
def get_embeddings(sentences, labels, word_embedding_file):
    ...
    # dict mapping labels (BC5 CDR) to integer indices
    label2idx = {
                 'O': 1,
                 'B-CHEM': 2,
                 'I-CHEM': 3,
    ...
    for line in embedding_file:
        ...
        if len(word2idx) == 0:
            word2idx['PADDING'] = 0
            word2idx['UNKNOWN'] = 1
            vector = np.zeros(len(splits)-1, dtype=np.float32)
            word_embeddings.append(vector)
            vector = np.random.uniform(-0.25, 0.25, len(splits) - 1).astype(np.float32)
            word_embeddings.append(vector)

        key = word.lower()
        if key in words:
            # Ensure numeric dtype
            word_vector = np.array([float(num) for num in splits[1:]], dtype=np.float32)
            word_embeddings.append(word_vector)
            # Store lowercase key for consistent lookup (dataset words were lowercased above)
