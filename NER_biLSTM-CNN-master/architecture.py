...
class CharacterEmbedding(layers.Layer):
    def __init__(self,
        ...
        self.chars_embedding_layer = layers.TimeDistributed(
            layers.Embedding(input_dim=len(self.char2idx),
                             ...
                             input_length=self.max_len_chars,
        ...
        self.pooling = layers.TimeDistributed(layers.MaxPool1D(pool_size=self.max_len_chars))
        self.flat = layers.TimeDistributed(layers.Flatten())

    def call(self, inputs, training=None):
        embedded_chars = self.chars_embedding_layer(inputs)
        x = self.dropout(embedded_chars, training=training)
    ...
    def get_config(self):
        ...
        config.update({
            ...
            'max_len_chars': self.max_len_chars,
...
class WordEmbedding(layers.Layer):
    def __init__(self, word2idx, word_embeddings, max_len_seq=50, name='Word_embedding', **kwargs):
        super().__init__(name=name, **kwargs)
...
def build_model(word2idx,
    ...
    word_input = layers.Input(shape=(max_len_seq,), dtype='int32', name='word_input')
    casing_input = layers.Input(shape=(max_len_seq,), dtype='int32', name='casing_input')
    char_input = layers.Input(shape=(max_len_seq, max_len_chars,), dtype='int32', name='char_input')

    embedded_words = WordEmbedding(word2idx=word2idx, word_embeddings=word_embeddings, max_len_seq=max_len_seq)(word_input)
    embedded_casing = CasingEmbedding(case2idx=case2idx, case_embeddings=case_embeddings)(casing_input)
    embedded_chars = CharacterEmbedding(char2idx=char2idx, max_len_chars=max_len_chars)(char_input)
